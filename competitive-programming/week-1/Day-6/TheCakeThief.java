...
public class Solution {
   ...
   public static int maxDuffelBagValue(final CakeType[] cakes, final int capacity)
    ...
    final int[] maxAmount = new int[capacity + 1];
    for (int i = 0; i <= capacity; i++) {
      ...
      for (int j = 0; j < cakes.length; j++) 
      {
        ...
        if (cake.weight <= i) {
          int maxValueUsingCake = cake.value + maxAmount[i - cake.weight];
        //   System.out.println("maxValueUsingCake: "+maxValueUsingCake);
          currentValue = Math.max(maxValueUsingCake, currentValue);
        //   System.out.println("currenValue: "+currenValue);
        }
      }
      maxAmount[i] = currentValue;
    //   System.out.println("maxAmount[i]: "+maxAmount[i]+"i value"+i+"  ");
    }
